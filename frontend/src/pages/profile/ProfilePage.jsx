...
export const ProfilePage = () => {
  ...
  const [uploadDocType, setUploadDocType] = useState('AADHAAR_CARD');
  const [uploadTitle, setUploadTitle] = useState('Aadhaar Card');
  ...
  const handleDocTypeChange = (e) => {
    ...
    if (selected) {
      if (val === 'OTHER') {
        ...
        setUploadTitle(selected.defaultTitle);
      }
    }
  };
  ...
  const handleUploadDocument = async (e) => {
    ...
    const selected = DOCUMENT_TYPE_OPTIONS.find((opt) => opt.value === uploadDocType) || {
      category: 'OTHER',
      value: 'OTHER',
      defaultTitle: 'Document',
    };

    const finalTitle = uploadTitle.trim() || selected.defaultTitle || uploadFile.name;
            ...
            <div className="flex items-center justify-between py-1 border-b border-slate-100 dark:border-slate-800">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">
                {profile?.personalEmail ? (
                  <span className="font-mono text-slate-900 dark:text-white">{profile.personalEmail}</span>
                ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                    Document Title {uploadDocType === 'OTHER' ? <span className="text-rose-500">*</span> : <span className="text-slate-400 font-normal">(Optional)</span>}
                  </label>
                  <input
                    type="text"
                    placeholder={uploadDocType === 'OTHER' ? 'Enter document name (e.g. Relieving Letter)' : 'e.g. Document Title'}
                    value={uploadTitle}
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Your personal email address for self-service contact and recovery. Your official company email remains unchanged.
            </p>
          </div>
